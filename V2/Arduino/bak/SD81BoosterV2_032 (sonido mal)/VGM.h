#define VGM_buffer_size 512
uint32_t ti_VGM = 0;
boolean playing_VGM = false;
uint8_t VGM_mode = 0;
SdFile VGMFile;
uint8_t retries = 0;
char VGM_buffer[VGM_buffer_size];
uint32_t VGM_buffer_pos = 0;
uint32_t VGM_buffer_len = 0;
...
uint8_t openVGM(char* s){
  ...
  VGM_buffer_pos = 0;
  VGM_buffer_len = VGMFile.read(VGM_buffer, VGM_buffer_size);
...
int16_t inline VGM_read(){
  int16_t result;
  if (VGM_buffer_pos >= VGM_buffer_len){
    VGM_buffer_len = VGMFile.read(&VGM_buffer, VGM_buffer_size);
    VGM_buffer_pos = 0;
  }
  if (VGM_buffer_len > 0) {
    result = (uint8_t) VGM_buffer[VGM_buffer_pos];
    VGM_buffer_pos++;
  } else result = -1;
  return result;
}
...
//  log_0("pos=%lu len=%lu", VGM_buffer_pos, VGM_buffer_len);
  // result = VGMFile.read();
  result = VGM_read();
//  log_0("pos=%lu len=%lu result=%02X", VGM_buffer_pos, VGM_buffer_len, result);

  if (result >=0) retries = 0;
  
 // log_0("VGM command %02X", result);
...
//   if (((result >= 0x40) && (result <= 0x4E)) || ((result >= 0x51) && (result <= 0x5F)) || ((result >= 0xB0) && (result <= 0xBF)) || ((result >= 0xA1) && (result <= 0xAF))){ // 2 bytes
//     log_0("VGM command %02X", result);
//   }
...
//   }
  ...
  switch (result) {
    ...
    case 0x61:              // Wait n samples, n can range from 0 to 65535 (approx 1.49 seconds). Longer pauses than this are represented by multiple wait commands. (44100samp/s)
      ...
      samples = (VGM_read()+256*VGM_read());//*3/2;
    ...
    case 0xA0:             // AY8910, write value dd to register aa
      ...
      if (result >= 0) {
        ...
        if (result >= 0){ 
          ...
          log_0("Error reading value");
      } else
        log_0("Error reading register");
      if (result < 0) {
        retries++;
        if (retries > 10){
          VGMFile.close();
