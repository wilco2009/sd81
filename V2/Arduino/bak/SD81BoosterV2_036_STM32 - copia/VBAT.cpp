...
bool vbat_init(void){
  log_3("Iniciando lectura de voltaje externo en el pin PC1...");
  ...
  if (HAL_ADC_Init(&hadc1) != HAL_OK) {
    log_0("ERROR: Fallo al inicializar el ADC.");
    return false;
  }
  log_3("ADC1 inicializado correctamente.");
  ...
  if (HAL_ADC_ConfigChannel(&hadc1, &sConfig) != HAL_OK) {
    log_0("ERROR: Fallo al configurar el canal ADC externo (PC1).");
    return false;
  }
  log_3("Canal ADC externo (PC1) configurado.");
...
float get_battery_level(void){
  ...
  if (poll_status == HAL_OK) { // Si la conversión fue exitosa
    ...
    log_0("Error: Conversion ADC no completada. Estado de poll: ");
    if (poll_status == HAL_TIMEOUT) {
      log_0("Tiempo de espera agotado (HAL_TIMEOUT).");
    } else if (poll_status == HAL_ERROR) {
      log_0("Error general (HAL_ERROR). Codigo: %d \n\r",hadc1.ErrorCode);
   } else {
      log_0("Estado desconocido.\n\r");
    }
  ...
  log_3("Lectura ADC cruda (PC1): %d  -> Voltaje Bateria: %f V\n\r",adc_raw_value,battery_voltage);
