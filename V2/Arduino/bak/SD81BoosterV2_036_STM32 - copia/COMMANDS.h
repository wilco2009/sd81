...
uint32_t fsize2;
  ...
  if (T81_dir){   // T81 dir
    ...
    Sfile.seekSet(0);
    if (!opened || (fsize>200000) || (fsize == 0)) {
      log_0("can't open load file");
      // send file length = 0
      SendByteToZ80(0);
      SendByteToZ80(0);
...
void cmd_AY_set_reg(){
  ...
  // log_3("REG=%hu VAL=%hu",reg,value);
  // log_3("ENV_E=%hu A_EN_EN=%hu A_EN=%hu E_t=%hu", AY_env.enabled, AY_config[0].env_enabled, AY_config[0].tone_enabled, AY_env.typ);
  // ToggleClock();              // Final clock toggle
  // reset_commands();
}
...
command_handler commands[] = {
  ...
  cmd_enable_Std_48K,   //48
  cmd_disable_Std_48K,   //49
  cmd_spare
};
