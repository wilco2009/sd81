...
//    sscanf(tmp,"/MAN/%s",fname);
  ...
  if (!opened || (fsize == 0)) {
    ...
    for (int i=0; (i < fsize);i++){
      ...
      // }      
      if (filetype==0)
        SendByteToZ80_IT(pgm_read_byte(asc_to_asc81+ch)); 
      else { 
...
char ascSize[0x10];
  ...
  if (T81_dir){
    ...
    for (int i=0; s[i]!=0;i++){
      ...
      SendByteToZ80_IT(pgm_read_byte(asc_to_asc81+s[i])); 
      ToggleClock();       // finish command
    }
    nfiles = 0;
    ndirs=0;
...
void cmd_rtc(){
  ...
  if (param_len==0) { // returns date time
    ...
    for (int i=0; i<param_len;i++){
      SendByteToZ80(pgm_read_byte(asc_to_asc81+date_time[i])); 
    }
...
void cmd_bat(){
  ...
  for (int i=0; i<5;i++){
    Serial.println(i);
    SendByteToZ80(pgm_read_byte(asc_to_asc81+sbat[i])); 
  }
  Serial.println("fin");
  SendByteToZ80(0x00); // ... and Status
  ToggleClock();       // Final clock toggle
