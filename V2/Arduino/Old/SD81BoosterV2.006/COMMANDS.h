...
char file_name[MAX_FILENAME_LEN];
  ...
  for (uint8_t i=0; i<param_len; i++){
    ToggleClock();
    ch = GetByteFromZ80();
    param1[i] = (char) pgm_read_byte(asc81_to_ascii+ch);
  }
  param1[param_len] = 0;
...
//  split_two_params(params, param1, param2);
  ...
  if (!sd.exists(param1))
    ...
    error_code = 4;
  else if (!sd.rename(param1,param2)){
    sprintf_P(tmp,PSTR("can not rename file %s to %s"),param1,param2);
    log_0(tmp);
    error_code = 3;
  }
  SendByteToZ80(error_code);  // ... and Status
...
//  split_two_params(params, param1, param2);
  Serial.print("FILE1: ");Serial.println(param1);
  ...
  complete_dir(param2,tmp);
  uint32_t i;
  boolean opened = Sfile.open(param1,O_READ);
  i = Sfile.fileSize();
