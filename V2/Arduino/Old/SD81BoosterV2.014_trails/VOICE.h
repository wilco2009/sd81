...
void voice_init() {
  ...
  Serial.println(F("VOICE EMULATOR Begin"));
  //serial_alofones[0] = 0;
...
void showHex(unsigned char* p){
  sprintf_P(str1, PSTR("BANK:%02X AL:%02X P:%lu SP:%lu Size:%lu EP:%lu "), voice_bank, cur_alofone, playing_pos, alofone_start_position(voice_bank, cur_alofone), alofone_size(voice_bank, cur_alofone), alofone_end_position(voice_bank, cur_alofone));
  Serial.println(str1);
...
void load_alofone_buffer(){
  ...
  #ifdef LOG3
  sprintf_P(str1, PSTR("BYTES LEFT: %lu, BUFFER SIZE: %d, DAC BUFFER CNT: %u"), bytes_left, DAC_BUFFER_SIZE, DAC_work_buffer_count);
  Serial.println(str1);
  #endif
  if (bytes_left > DAC_BUFFER_SIZE-DAC_work_buffer_count){
    fonema_file.seekSet(playing_pos);
    uint16_t result = fonema_file.read(DAC_work_buffer+DAC_work_buffer_count, DAC_BUFFER_SIZE-DAC_work_buffer_count);
    #ifdef LOG3
    showHex(DAC_work_buffer);
    #endif
    playing_pos+= DAC_BUFFER_SIZE;
    DAC_work_buffer_count+= result;
  } else {
    fonema_file.seekSet(playing_pos);
    uint32_t result = fonema_file.read(DAC_work_buffer+DAC_work_buffer_count, bytes_left);
    #ifdef LOG3
