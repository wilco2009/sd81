...
uint32_t fsize2;
  ...
  if (T81_dir){
    ...
    if (!opened || (fsize>65535) || (fsize == 0)) {
      ...
      result = Sfile.read(copy_buffer,127);
      if (strcmp(ext,"P")==0){
...
bool T81dir = false;
  ...
  if (isdir) {
     strcpy(dirname, s);
     strcpy(wildcards,"*");
     log_0("1: %s : %s", dirname, wildcards);
  } else {
    strcpy(tmp, current_dir);
    tmp[strlen(tmp)-1]=0;
    T81dir = (strcmp(get_filename_ext(tmp),"T81")==0);
    if (T81dir) {
      strcpy(dirname,tmp);
      strcpy(wildcards,params);
      log_0("2: %s : %s", dirname, wildcards);
    } else {
  ...
  if (!dir.open(dirname)){
    ...
    log_0("dir open failed: %s",dirname);
    error_code = 1;
  } else {
    if (!dir.isDir()) {
      T81dir = (strcmp(get_filename_ext(dirname),"T81")==0);
    ...
    if (T81dir) {
      ...
      delay(50);
      dir.open(dirname);
      dir.seekSet(4); // skip header
      while (!fin) {
        set_SDLed((millis() % 64)>32);
        if (dir.read(file_name,0x20)!= 0x20) break;
        char ascSize[0x10];
        if (dir.read(ascSize,0x10)!= 0x10) break; 
