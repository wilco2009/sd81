...
void AY_run(void){
  ...
  AY_set_volume(AY_CH_A, AYRegToVol(AY_reg[R_AMPLITUDE_A] & 0x0F));
  AY_set_volume(AY_CH_B, AYRegToVol(AY_reg[R_AMPLITUDE_B] & 0x0F));
  AY_set_volume(AY_CH_C, AYRegToVol(AY_reg[R_AMPLITUDE_C] & 0x0F));
}
    ...
    if (i < snote_len[channel]) {
      switch (s[i]){
        ...
        case 'V': // volumen por implementar (0..15)
                  ...
                  AY_set_volume(channel, AYRegToVol(AY_reg[10+channel] & 0x0F));
                  break;
        case ' ': 
        case 'n': 
