...
void AY_set_tone(uint8_t ch, uint16_t tone_freq) {
  ...
  uint16_t period = freqToPeriod(tone_freq);
  AY_reg[rfine[ch]] = period & 0xff;
  AY_reg[rcoarse[ch]] = (period >> 8) & 0xff;
...
void AY_SetReg(uint8_t reg, uint8_t value){
  ...
  switch (reg) {
    ...
    case R_COARSE_TUNE_B:
      AY_set_tone(1,AYRegToTone(AY_reg[R_FINE_TUNE_A], AY_reg[R_COARSE_TUNE_A]));
      break;
    case R_FINE_TUNE_C:
    case R_COARSE_TUNE_C:
      AY_set_tone(2,AYRegToTone(AY_reg[R_FINE_TUNE_A], AY_reg[R_COARSE_TUNE_A]));
      break;
    case R_NOISE_PERIOD:
      AYRegToNoise(AY_reg[R_NOISE_PERIOD]);
      break;
    case R_ENABLE: 
      if (bitRead(AY_reg[R_ENABLE],0)==1) AY_config[AY_CH_A].tone_enabled = true; else AY_reset_tone(AY_CH_A);
      if (bitRead(AY_reg[R_ENABLE],1)==1) AY_config[AY_CH_B].tone_enabled = true; else AY_reset_tone(AY_CH_B);
      if (bitRead(AY_reg[R_ENABLE],2)==1) AY_config[AY_CH_C].tone_enabled = true; else AY_reset_tone(AY_CH_C);
      if (bitRead(AY_reg[R_ENABLE],3)==1) AY_set_noise(AY_CH_A, AYRegToNoise(AY_reg[R_NOISE_PERIOD])); else AY_reset_noise(AY_CH_A);
      if (bitRead(AY_reg[R_ENABLE],4)==1) AY_set_noise(AY_CH_B, AYRegToNoise(AY_reg[R_NOISE_PERIOD])); else AY_reset_noise(AY_CH_B);
      if (bitRead(AY_reg[R_ENABLE],5)==1) AY_set_noise(AY_CH_C, AYRegToNoise(AY_reg[R_NOISE_PERIOD])); else AY_reset_noise(AY_CH_C);
      break;
    case R_AMPLITUDE_A:
    ...
    if (i < snote_len[channel]) {
      switch (s[i]){
        ...
        case 'W': // Envelope (0..7)
                  ...
                  // AY_set_env(4,AYRegToEnv(AY_reg[R_FINE_EP],AY_reg[R_COARSE_EP])); // no se porque hay que forzar esto
                  AY_set_env(AY_reg[R_ENV_SHAPE],AYRegToEnv(AY_reg[R_FINE_EP],AY_reg[R_COARSE_EP]));
