...
uint16_t note_len[9] = {2,3,4,6,8,12,16,24,32};
...
void Play_AY_channel(char* s, int channel){
  int i;
  i = index[channel];
...
//  Serial.println(S);
  if (!nota[channel]){
    switch (s[i]){
      ...
      case 'T': // tempo (negras por minuto)(60..240) por implementar
                ...
                //Serial.print("TEMPO:");Serial.println(cur_tempo);
                break;
      case 'h': 
      ...
      case '9': 
                if (octave[channel]) {
                  if (s[i] < '9') cur_octave[channel] = s[i]-'0';
                  else cur_octave[channel] = 8;
                  octave[channel] = false;
                } else if (s[i]>'0'){
                  if ((s[i+1] >= '0') && (s[i+1] <= '0')) {
                    // tresillos
                    uint8_t l = (s[i]-'0')*10 + (s[i+1]-'0');
                  } else cur_delay[channel] = cur_tempo*(note_len[s[i]-'1'])/8;
                }
                break;
  ...
  if (nota[channel]){
    if ((millis()-ini_time[channel]) > cur_delay[channel]) {
      i++;
      nota[channel] = false;
...
void Play_AY(char* p){
  cur_delay[0] = cur_delay[1] = cur_delay[2] = cur_tempo*(note_len[8])/8;
  ...
  while (!finished[0] || !finished[1] || !finished[2]) {
    for (int channel=0;channel<3;channel++){
      if (!finished[channel])
        Play_AY_channel(s[channel], channel);
    }
  }
  AY_stop();
}
