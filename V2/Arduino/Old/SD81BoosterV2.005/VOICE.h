...
int alofone_size(byte bank, byte alofone){
...
void load_alofone_buffer(){
  ...
  uint16_t bytes_left = (alofone_end_position(voice_bank, cur_alofone)-playing_pos)+1;
  sprintf_P(str1, PSTR("BYTES LEFT: %d, BUFFER SIZE: %d"), bytes_left, DAC_BUFFER_SIZE);
  Serial.println(str1);
  if (bytes_left > DAC_BUFFER_SIZE){
    fonema_file.seekSet(playing_pos);
    fonema_file.read(DAC_work_buffer+DAC_work_buffer_count, DAC_BUFFER_SIZE);
