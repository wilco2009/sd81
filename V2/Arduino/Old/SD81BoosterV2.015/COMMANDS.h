...
//  SendByteToZ80(error_code);  // ... and Status
  log_2("128 chars mode selected",0);
...
void cmd_sel_64_chars(){
  log_2("64 chars mode selected",0);
...
// COMMAND = 29
void cmd_fullpaging(){
  log_2("full paging mode selected",0);
...
// COMMAND = 30
void cmd_halfpaging(){
  log_2("half paging mode selected",0);
...
uint32_t i;
  ...
  //GET PARAMS
  uint8_t VGM_mode = GetByteFromZ80_IT();
  uint8_t param_len = GetByteFromZ80_IT();
  char ch;
  for (uint8_t i=0; i<param_len; i++){
    ToggleClock();
...
void cmd_cont_VGM() {
uint8_t error_code;
  log_2("Continue VGM",0);
  if (VGMFile.isOpen()) error_code = 0;
  else error_code = 12;
  if (error_code == 0) {
    set_SDLed(LED_ON);
    playing_VGM = true;
    log_3("VGM restarted",0);
  }
//  SendByteToZ80(error_code);  // ... and Status
  ToggleClock();
