   ...
   // El bucle interno procesa comandos de configuración y saltos de bucle sin demora.
    ...
    Serial.print(", Event Duration: "); Serial.print(event_duration_ms); Serial.println("ms");
        ...
        switch (cmd_char) {
            ...
            case 'H': { // Halt (detener la reproducción global)
                channel->current_index++; 
                play_parser_stop();
#ifdef debug
            ...
            case 'N': { // Separador numérico, ignorar
                channel->current_index++; 
#ifdef debug
            ...
            case 'W': { // Activar envolvente de hardware para este canal
                channel->current_index++;
                value = atoi(&channel->command_string[channel->current_index]);
                if (value >= 0 && value <= 15) {
            ...
            case 'U': { // Desactivar envolvente de hardware para este canal
                channel->current_index++;
            ...
            case 'X': { // Setear periodo de envolvente (GLOBAL)
                channel->current_index++;
                value = atoi(&channel->command_string[channel->current_index]);
...
uint8_t play_parser_start(const char* command_A, const char* command_B, const char* command_C) {
    log_2("Starting PLAY command parsing (3 voices)...\n\r");
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        log_2("  Channel %c",(char)('A' + i));
        log_2(" command: '%s'",parser_state.channels[i].command_string);
    }
