...
SerialComm::SerialComm(File &s, int stream_buffer_size)
: m_serial(s)
...
void SerialComm::rewind(){
  serial().seek(0);
}

int SerialComm::progress(){
  uint32_t ss = serial().size();
  uint32_t pp = serial().position();
  uint32_t rr = 100 * pp/ss;
  return rr;
}
